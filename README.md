...
📘 Flipbook Generator Setup & Usage Guide
...
Poppler is already bundled inside /flipbook-generate/:
...
Notes:
These values are reused for every issue
Only update once per magazine
⚠️ Important Note (Multiple Magazines)

Each magazine should have its own setup.

If you have multiple publications:

/Magazine-A/
/Magazine-B/
/Magazine-C/

Each should contain its own:

flipbook-generate/
library.json
PDF files

👉 Run the generator separately in each directory
...
├── spring-2026.pdf
📌 About library.json
Keeps track of all archived issues
The script will:
preserve existing issues
add the new issue to the top
2. Open Command Line
...
Each issue is self-contained (easy deployment)
